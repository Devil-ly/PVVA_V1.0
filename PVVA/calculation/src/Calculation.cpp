...
Calculation::Calculation(double f, double theta, double theta_h0, double z0,int N, int M)
	:f(f),theta(theta), theta_h0(theta_h0),z0(z0),N(N),M(M)
{
	lamda = C_Speed / f;
	k = 2 * Pi * f / C_Speed;
	ds = lamda / 3.5;
...
void Calculation::Initialization()
{
	...
	//旋转所用参数 旋转后free
	cosA = Allocate_2D(cosA, N2, M2);
	cosB = Allocate_2D(cosB, N2, M2);
	cosC = Allocate_2D(cosC, N2, M2);

	Vx = Allocate_2D(Vx, N2, M2);
	Vy = Allocate_2D(Vy, N2, M2);
	Vz = Allocate_2D(Vz, N2, M2);

	Vx1 = Allocate_2D(Vx1, N2, M2);
	Vy1 = Allocate_2D(Vy1, N2, M2);
	Vz1 = Allocate_2D(Vz1, N2, M2);

	i1 = Allocate_2D(i1, N2, M2);
...
void Calculation::Rotate()
{
	for (int i = 0; i < N2; i++)
	{
		for (int j = 0; j < M2; j++)
		{
			...
			cosA[i][j] = lamda * (i - 0.5 * 2 * (N - 1)) / ds / (2 * N - 1);
			cosB[i][j] = lamda * (j - 0.5 * 2 * (N - 1)) / ds / (2 * N - 1);
			cosC[i][j] = 1 - cosA[i][j] * cosA[i][j] - cosB[i][j] * cosB[i][j];
			if (cosC[i][j] < 0) cosC[i][j] = -2;
			else cosC[i][j] = pow(cosC[i][j], 0.5);

			Vx1[i][j] = cosA[i][j] / lamda;
			Vy1[i][j] = cosB[i][j] / lamda;
			Vz1[i][j] = cosC[i][j] / lamda;

			Vx[i][j] = cos(theta) * Vx1[i][j] + sin(theta) * Vz1[i][j];
			Vy[i][j] = Vy1[i][j];
			Vz[i][j] = cos(theta) * Vz1[i][j] - sin(theta) * Vx1[i][j];

			if ((cosC[i][j] != -2) && (Vz[i][j] >= 0))
			{
				i1[i][j] = Vx[i][j] * ds *(2 * N - 1) + (N - 1);
			}
			else
			{
				i1[i][j] = 10;
...
void Calculation::InserExEy()
{
	...
	for (int i = 0; i < 2 * N - 1; i++)
	{
		for (int j = 0; j < 2 * N - 1; j++)
		{
			int tempi1;
			tempi1 = int(i1[i][j]);
			AExTR[i][j] = InserVal(i1[i][j], Ex0[tempi1 - 1][j].real(), Ex0[tempi1][j].real(), Ex0[tempi1 + 1][j].real());
			AExTI[i][j] = InserVal(i1[i][j], Ex0[tempi1 - 1][j].imag(), Ex0[tempi1][j].imag(), Ex0[tempi1 + 1][j].imag());

			AEyTR[i][j] = InserVal(i1[i][j], Ey0[tempi1 - 1][j].real(), Ey0[tempi1][j].real(), Ey0[tempi1 + 1][j].real());
			AEyTI[i][j] = InserVal(i1[i][j], Ey0[tempi1 - 1][j].imag(), Ey0[tempi1][j].imag(), Ey0[tempi1 + 1][j].imag());

			if (cosC[tempi1][j] != -2)
			{
				if (Vz1[i][j] < 10e-6)
				{
					AExTR[i][j] = AExTR[i - 1][j];
					AExTI[i][j] = AExTR[i - 1][j];
					AEyTR[i][j] = AEyTR[i - 1][j];
					AEyTI[i][j] = AEyTR[i - 1][j];
				}
				else
				{
					AExTR[i][j] = abs(AExTR[i][j] * Vz[i][j] / Vz1[i][j]);
					AExTI[i][j] = abs(AExTI[i][j] * Vz[i][j] / Vz1[i][j]);
					AEyTR[i][j] = abs(AEyTR[i][j] * Vz[i][j] / Vz1[i][j]);
					AEyTI[i][j] = abs(AEyTI[i][j] * Vz[i][j] / Vz1[i][j]);
				}
			}
			else
			{
